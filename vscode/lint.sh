...
files=$(find ./src -name '*.ts' -and -not -name '*.d.ts')
if [ ${files[@]} ]; then
...
if grep -qr --include="*.tsx" propTypes src/; then
	echo "Please use type parameters instead of propTypes:"
	grep -rn --include="*.tsx" propTypes src/
	exit 1
fi

if grep -qr --include="*.tsx" 'className={`' src/; then
	echo "Please use the classNames helper instead of template strings:"
	grep -rn --include="*.tsx" 'className={`' src/
	exit 1
fi

if grep -qr --include="*.tsx" ' from "\.' src/; then
	echo "Please use absolute import paths:"
	grep -rn --include="*.tsx" ' from "\.' src/
	exit 1
fi
